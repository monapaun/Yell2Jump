...
public class YellRage extends LayoutGameActivity implements AudioChunkReadyEvent, CameraInfo, IOnMenuItemClickListener, IOnAreaTouchListener{
    /** Called when the activity is first created. */
	private volatile MicrophoneReadThread mrThread; // thread (Mr. Thread hehe) that controls the microphone recording
...
//	private final int COUNTDOWN_FONT_SIZE = 52;
	...
	private final int filterLength = 15;		//filter methods
	private short[] runningData;
	private int Qpos;
	...
	public Engine onLoadEngine() {
		...
		int filterLength = 15;	//filter bluh
		runningData = new short[filterLength];
		for (int i=0; i < filterLength; i++) runningData[i] = 0;
		Qpos = 0;
		runningAverage = 0;
...
//			countdownText[i].setPosition(CAMERA_WIDTH/2f - countdownText[i].getWidth()/2f, CAMERA_HEIGHT/2f - COUNTDOWN_FONT_SIZE/2f);
		...
		{
			wheel[i] = new Wheel(-50, -50, YellRage.this.wheelTextureRegion);
		}
			...
			public void onUpdate(final float pSecondsElapsed) {
				...
				for (int j=1; j<=10; j++){
					if(player.collidesWith(wall[j])) {
						if (YellRage.this.mEngine.getSecondsElapsedTotal()-temp_collide>2f){
						...
						temp_collide=YellRage.this.mEngine.getSecondsElapsedTotal();
			...
			public void onTimePassed(final TimerHandler pTimerHandler) {
				...
				if (randY<=2) {
					for (int j=1; j<=randY+1; j++){
						...
						if (k>10) k=1;
						//Log.d("moo","contour"+k);
						wall[k] = new Wall (800+randX, 290-32*(j-1), YellRage.this.wallTextureRegion);
					...
					k++;
					if (k>10) k=1;
					wall[k] = new Wall (800+randX, 290, YellRage.this.wallTextureRegion);
					wall[k].setScale((float) 0.5);
					mScene.attachChild(wall[k]);
					k++;
					if (k>10) k=1;
					wall[k] = new Wall (800+randX, 290-32, YellRage.this.wallTextureRegion);
					wall[k].setScale((float) 0.5);
					mScene.attachChild(wall[k]);
					k++;
					if (k>10) k=1;
					wall[k] = new Wall (800+randX+32, 290, YellRage.this.wallTextureRegion);
					wall[k].setScale((float) 0.5);
					mScene.attachChild(wall[k]);
					k++;
					if (k>10) k=1;
					wall[k] = new Wall (800+randX+32, 290-32, YellRage.this.wallTextureRegion);
					wall[k].setScale((float) 0.5);
					mScene.attachChild(wall[k]);
				} else if (randY==4) {
					k++;
					...
					k++;
					if (k>10) k=1;
					wall[k] = new Wall (800+randX+64, 290, YellRage.this.wallTextureRegion);
					wall[k].setScale((float) 0.5);
					mScene.attachChild(wall[k]);
					k++;
					if (k>10) k=1;
					...
					k++;
					if (k>10) k=1;
					wall[k] = new Wall (800+randX+64, 290-32, YellRage.this.wallTextureRegion);
					wall[k].setScale((float) 0.5);
					mScene.attachChild(wall[k]);
				}
			}
		...
		if (setPauseOnLoad == true)
		{
			if (state.isSet() && state.player_isalive && state.player_life > 0 && state.world_secondselapsed > 0) // check the life just in case
			{
				...
				for (int i = 0; i < wall.length; i++)
				{
					...
					{
						wall[i] = new Wall (state.wall_x[i], state.wall_y[i], YellRage.this.wallTextureRegion);
...
//			hsTable.openDB();
	...
	public synchronized void startMCRThread(){
...
//						try
//						{
//							Log.d("MCR","Trying to load microphone");
//							mrThread = new MicrophoneReadThread(me);
//							mrThread.start();
//							Log.d("MCR","SUCCESS!!");
//							break;
//						}
...
//		}).start();
		  ...
		  if(mrThread == null){
			    mrThread = new MicrophoneReadThread(this);
			    mrThread.start();
			  }

	}

		public synchronized void stopMCRThread(){
		  if(mrThread != null){
			  MicrophoneReadThread tmpThread = mrThread;
		    mrThread = null;
		    tmpThread.close();
		  }
		}
